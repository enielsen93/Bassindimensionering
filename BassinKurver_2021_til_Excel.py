...
udlobstal = np.round(np.logspace(start = -2, stop = 8, num = 100, base = 2),2)
...
Vmax = np.empty((500, len(udlobstal)))
...
RP = np.sort((np.array([TimeSeriesDuration / float(1 + j) for j in range(Vmax.shape[0])])))
